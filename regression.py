...
class Trader:
    ...
    def predict_returns(self, bt_spread, 
        ...
        coef_bt_x_vol = 0.02346829709953024
        coef_bt_vol_ratio = -2.699556197092224e-05
        coef_bt_spread = 0.032974712289000854
        coef_bt_orderbook_imbalance = -8711.814720353581
        coef_bt_orderbook_imbalance_ratio = -18274.815361267443
        coef_bt_bid_flow = 24628748.533908647
        coef_bt_ask_flow = -24628748.5339088
        coef_bt_order_flow_imbalance = -24628748.533904165

        
        intercept = 18274.81532692434
    ...
    def starfruit(self, order_depth, statePos, prev_bid_ask_vol, prev_mid_price):
        ...
        features = [spread, normalized_imbalance, max_ratio, bid_flow, ask_flow, order_flow_imbalance]

        returns = self.predict_returns(*features)
        logger.print("returns: " + str(returns))
        #if returns > 0 and  > (ask- mid_price) / mid_price, then we would buy and sell if returns < (bid - mid_price)/ mid_price
        if returns*mid_price > (best_ask - best_bid):
            if self.position[prod] <= 20: # this is actually selling
                logger.print("buy")
                max_buy = 20 + (self.position[prod])
                logger.print(str(-1 * max_buy))
                orders['STARFRUIT'].append(Order('STARFRUIT', best_ask-2 , -1 * max_buy))
                alrBought += max_buy
        elif returns*mid_price < -1 *spread:
            if self.position[prod] >= -20: # this is actually buying
                max_sell = 20 - self.position[prod]
                orders['STARFRUIT'].append(Order('STARFRUIT', best_bid+2, max_sell))
                alrSold += max_sell
